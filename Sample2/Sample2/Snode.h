...
SC_MODULE(Snode) {
    ...
    Snode(sc_module_name name, int num_, WSAP cc_) : sc_module(name),
        ...
        cc(cc_), number(num_),
        sent_k(0), sent_shrq(0), sent_shpn(0), sent_lb(0),
    ...
    ~Snode()
    {
        ...
        {
            error = true;
            cout << "InternalError,Snode," << name() << ",delayed_k is not empty: " << delayed_k.size() << endl;
        }
        if (0 < delayed_lb.size())
        {
            error = true;
            cout << "InternalError,Snode," << name() << ",delayed_lb is not empty: " << delayed_lb.size() << endl;
        }
        if (!got_m)
        {
            error = true;
            cout << "Error,Snode," << name() << ",did not receive m_block" << endl;
        }
        if (is_Sleader && (total_msigns < cc->Anum))
        {
            error = true;
            cout << "Error,Snode," << name() << ",did not receive all msigns" << endl;
        }
        if (error)
        {
            cout << "Destructor,Snode," << name() << ",k," << kblk_rec << "," << kblk_snt <<
    ...
    void ProcessRoutingBlock(routing_block& rb);
    //! just process block according to its contents (routing part of routing block is ignored)
    void ProcessBlock(routing_block& rb);
    //! just send routing block (not processing its routing information) to finger f
    void SendBlock(routing_block& rb, int nodenum);
    //! broadcasting routine
    void ForwardBlock(routing_block& rb);

    void UpdateDiskInfo(double bitsWritten);
};
